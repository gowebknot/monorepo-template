...
The main application (`src/`) exposes health and production Better Auth. All example/reference modules (database, users, todos, todo-items, auth) live in `reference/` and can be run as a separate Nest application for demonstration or copy-paste purposes.
