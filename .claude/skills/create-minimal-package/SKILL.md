...
2. For new packages, run `pnpm package:create <project-name>` from the repo root instead of writing the package setup by hand; the script sets `name` to `@monorepo-template/<project-name>` and creates package-local `README.md`, `AGENTS.md`, and `CLAUDE.md` files. `CLAUDE.md` imports its sibling guidance with `@AGENTS.md`.
