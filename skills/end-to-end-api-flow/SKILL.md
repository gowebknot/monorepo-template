...
2. Start with `@repo/entities`. Define or update request, response, params, and error Zod schemas together with their inferred types. Export them through the package barrels; never recreate API shapes downstream.
...
4. Add the typed operation to `@repo/api-client`. Reuse shared paths and services, import types from `@repo/entities`, return `response.data`, and keep transport concerns out of consumers.
5. Add or update the corresponding `@repo/query-client` hook. Build it on the API-client operation, keep query keys and cache invalidation there, and do not duplicate HTTP calls or service logic.
