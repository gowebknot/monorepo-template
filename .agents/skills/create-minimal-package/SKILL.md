...
2. For new packages, run `pnpm package:create <project-name>` from the repo root instead of writing the package setup by hand; the script sets `name` to `@<root package.json name>/<project-name>` and creates package-local `README.md` and `AGENTS.md` files.
