...
- [x] The release commit and annotated `v0.1.57` tag are pushed to `origin` before publication.
- [x] The package-local wrapper publishes `create-mono-stack@0.1.57`; npm latest and remote tag
      confirm it.
...
- First observed run: 2026-09-13, before pushing, `origin/master` had no `v0.1.57` tag and
  `npm view create-mono-stack@latest version` returned `0.1.56`.
- Passing rerun: 2026-09-13, `git push origin master` (`4444f8e..6eb8129`), `git tag -a v0.1.57` then
  `git push origin v0.1.57` (`git ls-remote --tags origin v0.1.57` returned
  `c0c95b0df991a5ee59de76d49f476fe375f7e9db`, peeling via `git rev-parse v0.1.57^{}` to
  `6eb81294b7ec953259fb0269bf1d5630e1094a91`, equal to `HEAD`), then
  `pnpm --filter create-mono-stack publish:package` succeeded. `npm view create-mono-stack@latest
version` reported `0.1.57` after an ~8s poll.
...
- [x] Commit release metadata with hooks, push `master`, create/push annotated `v0.1.57`.
- [x] Publish with the package wrapper and record TEST-RELEASE-075 evidence.

## Validation Notes

- Registry baseline: `npm view create-mono-stack version` (before this release) was `0.1.56`.
- TEST-RELEASE-072: `pnpm --filter create-mono-stack test` passed at 0.1.57 (305/305).
- TEST-RELEASE-074: dry run produced `create-mono-stack-0.1.57.tgz`, 300 files, 619.2 kB packed,
  1.0 MB unpacked; excludes the package `test/` directory, `node_modules`, and `.npmrc*`.
- TEST-RELEASE-075: `v0.1.57` was pushed as annotated tag `c0c95b0df991a5ee59de76d49f476fe375f7e9db`,
  peeling to release commit `6eb81294b7ec953259fb0269bf1d5630e1094a91`. The package-local wrapper
  published `create-mono-stack@0.1.57`; npm reports that version as `latest`.

## Updates

### 2026-09-13 - Release published

- Release commit: `6eb81294b7ec953259fb0269bf1d5630e1094a91`
  (`chore(release): prepare create-mono-stack 0.1.57`), pushed to `origin/master`, preceded by the fix
  commit `ee935a8` (also pushed in the same `git push`).
- Tag: annotated `v0.1.57` (`c0c95b0df991a5ee59de76d49f476fe375f7e9db`) pushed to origin; peeled ref
  `v0.1.57^{}` is `6eb81294b7ec953259fb0269bf1d5630e1094a91`.
- Publication: `pnpm --filter create-mono-stack publish:package` completed successfully with public
  latest access.
- Verification: `npm view create-mono-stack@latest version` reports `0.1.57`.
